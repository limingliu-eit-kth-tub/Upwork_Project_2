...
void set_phy(int nRSU, int nVehicle, int guardInterval, NodeContainer &rsuNode, NodeContainer &vehicularNode, YansWifiPhyHelper &phy)
{
...
void set_nodes(int channelWidth, int rng, int32_t simSeed, NodeContainer rsuNode, NodeContainer vehicularNode, YansWifiPhyHelper phy, WifiMacHelper mac, WifiHelper wifi, NetDeviceContainer &rsuDevice)
{
...
void set_sim(bool tracing, bool dry_run, int warmup, uint32_t openGymPort, YansWifiPhyHelper phy, NetDeviceContainer rsuDevice, int end_delay, Ptr<FlowMonitor> &monitor, FlowMonitorHelper &flowmon)
{
 ...
 */
    ...
    ScenarioFactory helper = ScenarioFactory(nRSU, rsuNode, vehicularNode, port, offeredLoad, history_length);
    ...
    vanetSceanrio->installScenario(simulationTime + end_delay + envStepTime, envStepTime, MakeCallback(&packetReceived));
    ...
    set_sim(tracing, dry_run, warmup, openGymPort, phy, rsuDevice, end_delay, monitor, flowmon);
    ...
    */
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin(); i != stats.end(); ++i)
    {
        ...
        myfile << std::put_time(&tm, "%Y-%m-%d %H:%M") << "," << CW << "," << nRSU << "," << RngSeedManager::GetRun() << "," << t.sourceAddress << "," << t.destinationAddress << "," << flowThr;
        myfile << std::endl;
    }
    myfile.close();
